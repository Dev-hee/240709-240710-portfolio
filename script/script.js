...
// swiper 클래스 대상의 안에 작성 했을 땐 swiper가 absolute를 잡는 relative 기본내장으로 별도 설정을 안해도 되지만 밖에 작성했을 땐 밖 위치 기준으로 그 부모에게 relative설정을 다로 해줘야 한다.

//sns swiper
const snsS = new Swiper('.sns .swiper', {
    slidesPerView:2, //한번에 보일 슬라이드 수
    spaceBetween:20, //슬라이드 사이 여백
    autoplay:{delay:1,desableOnInteration:true,}, //딜레이 없이 빠르게 넘어가게 하는건 1
    loop:true, // 무한반복 
    speed:8000,  //8초
    freemode:true, //사용자가 이동하는 방향으로 흘러가게
    //반응형 슬라이드 수
    breakpoints:{
        800:{slidesPerView:3,},//800이상일 떄 3개
        1200:{slidesPerView:4,},//800이상일 떄 3개
    }
})
//detail swiper
const detailS = new Swiper('.detail .swiper', {
    slidesPerView:4,//한번에 보일 슬라이드 수
    spaceBetween:20,//슬라이드 사이 여백
    autoplay:{delay:2000}, //딜레이 없이 빠르게 넘어가게 하는건 1
        //반응형 슬라이드 수
        breakpoints:{
            800:{slidesPerView:3,},//800이상일 떄 3개
            1200:{slidesPerView:4,},//800이상일 떄 3개
...
big_bg.style.display = 'none' //팝업초기 숨기기
